...
plot_dir <- "plots/calibration_plots_conditional_mean"
n_bins <- 3
...
for (l in 0:21) {
    ...
    ens <- fc_data$ens
    ens_mean <- rowMeans(ens, na.rm = TRUE)
    #create bins based on the mean with approximately equal counts in each bin
    bin_breaks <- quantile(ens_mean, probs = seq(0, 1, length.out = n_bins + 1), na.rm = TRUE)
    bin_breaks <- unique(bin_breaks)
    # Add small offset to the maximum value to include it in the last bin
    bin_breaks[length(bin_breaks)] <- bin_breaks[length(bin_breaks)] + 1e-10
    bins <- cut(ens_mean, breaks = bin_breaks, labels = 1:n_bins, include.lowest = TRUE)

    for (method in toupper(names(fc_data)[2:(length(names(fc_data)))])) {
        for (i in 1:n_bins) {
            
            fc_data_bin <- lapply(names(fc_data), function(x) {
                if (x == "ens") {
                    return(ens[bins == i, ])
                } else if (x == "hen") {
                    return(list(bin_edges = fc_data$hen$bin_edges[bins == i], f = fc_data$hen$f[bins == i]))
                } else {
                    return(fc_data[[x]][bins == i, ])
                }
            })
            names(fc_data_bin) <- names(fc_data)

            obs_vec_bin <- obs_vec[bins == i]

            print(paste0("Processing ", method, " at lead time ", l))
            c <- plot_postproc(method, obs_vec_bin, fc_data_bin, lead_time = l, tvals = t_vals, type = "plot", plot_dir = plot_dir)
            #crps_df <- rbind(crps_df, data.frame(method = method, lead_time = l, t = t_vals, twcrps = c$twc, owcrps = c$owc))
            #calibration_abs_dist <- rbind(calibration_abs_dist, data.frame(method = method, lead_time = l, t = t_vals, com = c$com_dist, sev = c$sev_dist))
        }
    }
