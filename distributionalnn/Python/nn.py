    ...
    for train_no in range(val_multi):
        ...
        if paramcount[distribution] is None:
            ...
            if distribution == 'Normal':
                #outputs = GaussianLayer(24)(linear)
...
params_jsu_1 = pd.read_csv("Datasets/nn_params_normal_1.csv")

params_jsu_1_dict = params_jsu_1.set_index('param_name')['param_value'].to_dict()
params = params_jsu_1_dict
...
def build_best_model(params):
    inputs = keras.Input(shape=(X_pred.shape[1],))
    last_layer = inputs
    ...
    h1_activation_rate = params['h1_activation_rate_l1'] if not params['regularize_h1_activation'] else 0.0
    h1_kernel_rate = params['h1_kernel_rate_l1'] if not params['regularize_h1_kernel'] else 0.0
    hidden = keras.layers.Dense(params['neurons_1'],
                                activation='softplus', #params['activation_1'],
                                kernel_regularizer=keras.regularizers.L1(h1_kernel_rate),
                                activity_regularizer=keras.regularizers.L1(h1_activation_rate))(last_layer)

    h2_activation_rate = params['h2_activation_rate_l1'] if not params['regularize_h2_activation'] else 0.0
    h2_kernel_rate = params['h2_kernel_rate_l1'] if not params['regularize_h2_kernel'] else 0.0
    hidden = keras.layers.Dense(params['neurons_2'],
                                activation='elu', #params['activation_2'],
                                kernel_regularizer=keras.regularizers.L1(h2_kernel_rate),
                                activity_regularizer=keras.regularizers.L1(h2_activation_rate))(hidden)
    ...
    if paramcount[distribution] is None:
        ...
        param_names = ["loc", "scale", "tailweight", "skewness"]
        for p in range(paramcount[distribution]):
            param_kernel_rate = params[param_names[p]+'_rate_l1'] if not params['regularize_'+param_names[p]] else 0.0
            param_layers.append(keras.layers.Dense(
...
best_model = build_best_model(params)
callbacks = [keras.callbacks.EarlyStopping(patience=200, restore_best_weights=True)]

best_model.fit(Xwhole[:1092], Ywhole[:1092], validation_data = (Xwhole[1092:], Ywhole[1092:]), epochs=1500, batch_size=32, verbose=True, callbacks=callbacks)

# Predict with the best model
predictions = best_model.predict(X_pred)
print(predictions)

y_hat = best_model(X_pred)
